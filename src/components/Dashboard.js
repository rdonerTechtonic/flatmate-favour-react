...
export const Dashboard = ({ ffRoommates, ffEvents, handleUpdateEventStatus, handleDateChange, handleEditHouse, handleEventEdit }) => {
    return (
      <div>
        <Link to="/login"><img src="/Flatmate-Favour-Logo.png" alt="flatmate favour icon" id="logo" /></Link>
          <UserBar handleEditHouse={handleEditHouse} ffRoommates={ffRoommates} />
            <EventDashboard ffEvents={ffEvents} handleUpdateEventStatus={handleUpdateEventStatus} handleDateChange={handleDateChange} handleEventEdit={handleEventEdit}/>
      </div>
    );
  };
